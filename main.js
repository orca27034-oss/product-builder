console.log("Dinner Recommender: Script starting...");
...
function initApp() {
    console.log("Dinner Recommender: Initializing app...");
    
    const chips = document.querySelectorAll('.chip');
    const recommendBtn = document.getElementById('recommend-btn');
    const resultSection = document.getElementById('result-section');
    const menuCategoryElem = document.getElementById('menu-category');
    const menuNameElem = document.getElementById('menu-name');
    const menuDescElem = document.getElementById('menu-desc');

    if (!recommendBtn || !resultSection) {
        console.error("Dinner Recommender ERROR: Required elements not found!");
        return;
    }
    ...
    chips.forEach(chip => {
        chip.addEventListener('click', () => {
            console.log("Category clicked:", chip.dataset.category);
    ...
    recommendBtn.addEventListener('click', () => {
        console.log("Recommend button clicked. Selected category:", selectedCategory);
        ...
        if (filteredMenus.length === 0) {
            console.warn("No menus found for category:", selectedCategory);
            return;
        }
        ...
        setTimeout(() => {
            // Update content
            menuCategoryElem.textContent = selectedMenu.categoryName;
            menuNameElem.textContent = selectedMenu.name;
            menuDescElem.textContent = selectedMenu.desc;
            ...
            console.log("UI updated and visible.");
        }, 300);
    });
    
    console.log("Dinner Recommender: Event listeners attached.");
}

// Support both DOMContentLoaded and defer
